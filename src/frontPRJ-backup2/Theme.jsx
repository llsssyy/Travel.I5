...
const Theme = () => {
    //이미지 리스트
    const imageList = [
        {id:1,  category: 'snow',     src: './resources/cra_ex/theme/ex/1.png',  content: '무주 \'덕유산 국립 공원\''},
        {id:2,  category: 'snow',     src: './resources/cra_ex/theme/ex/2.png',  content: '정선 \'만항제\''}, 
        {id:3,  category: 'forest',   src: './resources/cra_ex/theme/ex/3.png',  content: '원대리 자작나무 숲'},
        {id:4,  category: 'activity', src: './resources/cra_ex/theme/ex/4.png',  content: '설악 워터피아'},
        {id:5,  category: 'snow',     src: './resources/cra_ex/theme/ex/5.png',  content: '당진 신리성지'},
        {id:6,  category: 'forest',   src: './resources/cra_ex/theme/ex/6.png',  content: '메타세콰이어길'},
        {id:7,  category: 'beach',    src: './resources/cra_ex/theme/ex/7.png',  content: '서춘 춘장대 해수욕장'},
        {id:8,  category: 'forest',   src: './resources/cra_ex/theme/ex/8.png',  content: '옥녀 교차로'},
        {id:9,  category: 'beach',    src: './resources/cra_ex/theme/ex/9.png',  content: '하슬라'},
        {id:10, category: 'activity', src: './resources/cra_ex/theme/ex/10.png', content: '평창 \'용평 리조트 스키장\''},
        {id:11, category: 'beach',    src: './resources/cra_ex/theme/ex/11.png', content: '속초 영금정'},
        {id:12, category: 'activity', src: './resources/cra_ex/theme/ex/12.png', content: '알프스 마을'},
    ];
        ...
        dummyThemeDB = {
            ...
            ["forest"]: {
                ...
                info: "원대리에 위치한 자작나무 숲이다. 한국인이 꼭 가봐야할 '한국 관광 100선'에 선정된 곳으로, 우리나라 자작나무숲 중 최고라 해도 손색이 없다." 
                    ...
                    + " 겨울왕국에 햇살이 더해지니 눈부시게 반짝인다. 자연과 계절이 주는 아룸다운을 가득 느끼며 힐링 시간을 보내보자."
            },
            ["waterpark"]: {
                theme: "waterpark",
                name: "국내 최초이자 최대 온천 워터파크",
                info: "강원도 속초시에 위치한 워터파크다. 스파부터 다양한 어트랙션을 즐길 수 있어 겨울철에도 발길이 끊이질 않는 곳이다. 설악워터피아의 가장 큰 매력은 설악산 경치와 함께 할 수 있는" 
                    + " 노천온천이 아닐까. 이곳의 온천수는 지하 680미터 지점에서 하루 3천 톤씩 용출되는 천연 온천수라고 한다. 아름다운 자연을 바라보며 즐기는 스파. 더할 나위 없다." 
                    + " 설악워터피아 곳곳에서 신나는 어트랙션도 만나볼 수 있다. 다채로운 시설이 마련되어 있어 발견하는 재미가 있는 곳이가. 아이부터 어른까지" 
                    ...
                    + " 착용해야 이용이 가능하며 허기를 달래주는 푸드코트도 마련되어 있으니 참고할 것. 움츠러든 몸과 마음 모두 설악워터피아에서 풀어보자."
            },
            ["holyPlace"]: {
                theme: "holyPlace",
                name: "천주교의 대표적인 성지 중 하나",
            ...
            ["haseulla"]: {
                ...
                info: "어째선지 바닷가의 대형 카페가 연상되는 이름 하슬라. 하슬라는 강릉의 옛 이름이라고 한다. 동해 바다 앞에 위치한 강릉 하슬라아트월드에서는" 
                    ...
                    + " 에메랄드빛 바다를 눈에 담으며 쉬어갈 수 있다. 벽에 걸린 그림에는 도무지 감흥이 생기지 않는다면 강릉 하슬라아트월드를 추천한다. 포토존은 덤, 신비로운 설치미술의 세계에서 색다른 자극을 얻게 될 것이다."
            },
            ["activity"]: {
                theme: "activity",
                name: "겨울철 필수 액티비티",
            ...
            ["sogcho"]: {
                theme: "sogcho",
                name: "속초 영금정",
                info: "강원도 속초시 영금정로 43" 
                    + " 속초 영금정은 속초 등대 밑의 바닷가에 넓게 자리 잡은 바위군을 일컫는다. 파도가 바위에 부딪치면서 내는 소리가 거문고 소리와 비슷하여 영금정이라 부른다." 
            ...
            ["alpeuseu"]: {
                ...
                name: "알프스 마을",
                info: "충남 청양 알프스 마을" 
                    + " 명당 7곳을 품었다는 칠갑산은 충남의 알프스라고 불린다. 칠갑산 정산 바로 아래 칠갑산 기슭에 자리 잡은 알프스 마을은 천장처럼 높다는 의미로 천장리라고 부르며, 아름다운 천장 호수와 더불어 천장리" 
    ...
    //handler
    const themeModalthumsClickHandler = (pn) => {
    ...
    const modalBgClickHandler = () => {
        ...
        //모달이 보이지 않아야 하니까 true였던 값을 false로
    ...
    return(
        <div id="theme_wrap">
            ...
            <div className="theme_select_bar">
                <a href="#" onClick={() => handleCategorySelect('all')}>all</a>
                <a href="#" onClick={() => handleCategorySelect('snow')}>snow</a>
                <a href="#" onClick={() => handleCategorySelect('beach')}>beach</a>
                <a href="#" onClick={() => handleCategorySelect('forest')}>forest</a>
                <a href="#" onClick={() => handleCategorySelect('activity')}>activity</a>
            </div>
                ...
                <Slider style={sliderStyle} {...setting} className="img_slide">
                    ...
                    {filteredImages.map((image) => (
                        ...
                        <div key={image.id} className="thum">
                            {/* 이미지의 id값을 전달 */}
                            <a href="#none" onClick={() => themeModalthumsClickHandler(image.id)}>
                                {/* 이미지 소스의 경로 */}
                                <img src={image.src} />
                                {/* 이미지의 내용 */}
...

        {
            ...
            <>
                <div className="modal">
                    ...
                    <a href="#none" onClick={modalBgClickHandler} className="cancle_btn">
                        CLOSE
                    </a>
                </div>
            </>                  
