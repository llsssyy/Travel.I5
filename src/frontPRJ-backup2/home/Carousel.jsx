...
import 'swiper/css/navigation';                    // 라이브러리 css 파일(아래 2개도 마찬가지)
...
export default function RecomendSlider() {           // 라이브러리 통용 소스 시작
    ...
    const RecomendSpotClickHandler = (k) => {      // 3개 이미지 통으로 핸들러 만들어 분기 태우기                                                  
        setDbKey(k);                               // useState
        setshowModal(true);
        switch (k) {                               // 핸들러 분기 태우기(alpeuseu~ 킷값)
            case 'snow':
    ...
    const [swiperPlaybutton, setswiperPlaybutton] = useState(null);
    ...
    return (
        <Swiper
            ...
            autoplay={{ delay: 3000, disableOnInteraction: false, }}   // 처음부터 플레이, 바뀌는 시간
            onSwiper={setswiperPlaybutton}
        >
            {
                showModal
                ?
                <Modal keyValue={dbKey} modalViewer={setshowModal}/>
                :
                null
            }
            {/* a 태그와 img src 태그 */}
            ...
            <SwiperSlide><a href="#none" onClick={() => RecomendSpotClickHandler('snow')}>
                <img src={`resources/prj_imgs/carousel/recomend01.jpg`} />
            ...
            <SwiperSlide><a href="#none" onClick={() => RecomendSpotClickHandler('gangwon')}>
                <img src={`resources/prj_imgs/carousel/recomend02.jpg`} />
            ...
            <SwiperSlide><a href="#none" onClick={() => RecomendSpotClickHandler('incheon')}>
                <img src={`resources/prj_imgs/carousel/recomend03.jpg`} />
            ...
            <div className='start_pause_button_wrap'>
                <button type='button' className='start_button' onPointerUp={playButton}>▶</button>
                <button type='button' className='pause_button' onPointerUp={pauseButton}>❚❚</button>
            </div>
        </Swiper>
    )
}
